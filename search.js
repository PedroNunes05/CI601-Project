...
async function sendApi(chose) {
    sort = chose;
    if (sort == "none") {
        const response = await fetch(`https://pn163.brighton.domains/CI601/php/search.php?searchPage="true"`);
        const data = await response.json();
        const quizzes = data.quizzes;
        searchResponse(quizzes);
        //send the api and wait for a response
    } else {
        const response = await fetch(`https://pn163.brighton.domains/CI601/php/search.php?searchPage=true&sort=${sort}`);
        const data = await response.json();
        const quizzes = data.quizzes;
        searchResponse(quizzes);
	//send the api and wait for a response
